...
const StudentSubmittedCompliance = require("../models/StudentSubmittedCompliance");
...
const uploadFile = async (req, res) => {
  try {
    ...
    const { studentId, complianceName } = req.body;
    console.log(studentId, complianceName);
    if (!studentId || !complianceName) {
      return res
        .status(400)
        .json({ error: "Student ID and compliance name are required" });
    }
    ...
    await StudentSubmittedCompliance.submitCompliance(
      complianceName,
      studentId,
      result.secure_url
    );
...
const deleteFile = async (req, res) => {
  try {
    ...
    // Fetch file from DB
    const [files] = await StudentSubmittedCompliance.getFileById(id);
    if (files.length === 0) {
      return res.status(404).json({ error: "File not found" });
    }

    const fileUrl = files[0].compliance_pdf_url;
    ...
    try {
      ...
      await StudentSubmittedCompliance.deleteSubmission(id);
...
const updateFileStatus = async (req, res) => {
  ...
  const { id, status, wardenId } = req.body;
  if (!id || !status) {
    return res
      .status(400)
      .json({ success: false, message: "Student ID and status are required" });
  }
  ...
  try {
    ...
    await StudentSubmittedCompliance.updateStatusAndWarden(
      id,
      status,
      wardenId
    );
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to update file status" });
  }
};
...
const getAllSubmissionsWithStudentAndWardenName = async (req, res) => {
  try {
    const submissions = await StudentSubmittedCompliance.getAllSubmissionsWithStudentAndWardenName();
...
module.exports = { uploadFile, deleteFile, updateFileStatus ,getAllSubmissionsWithStudentAndWardenName};
